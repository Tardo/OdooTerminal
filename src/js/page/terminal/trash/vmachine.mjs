...
export default class VMachine {
  ...
  #callFunction(frame, parse_info, silent) {
    return new Promise(async (resolve, reject) => {
      ...
      if (cmd_def) {
        ...
        if (frame.args.length > items_len) {
          return reject(
            new Exceptions.InvalidCommandArgumentsError(frame.cmd, frame.args)
          );
        }
        let kwargs = {};
        ...
        try {
          ...
          return reject(
            new Exceptions.InvalidCommandArgumentFormatError(err, frame.cmd)
          );
        }
      }
      // Alias
          ...
          case INSTRUCTION_TYPE.CALL_FUNCTION:
            {
              ...
              try {
                ...
                return reject(new Exceptions.CallFunctionError(err));
              }
            }
            break;
          ...
          case INSTRUCTION_TYPE.STORE_NAME:
            {
              ...
              if (!vname) {
                if (!token) {
                  return reject(new Exceptions.InvalidInstructionError());
                }
                return reject(
          ...
          case INSTRUCTION_TYPE.STORE_SUBSCR:
            {
              ...
              try {
                ...
                return reject(new Exceptions.InvalidInstructionError(err));
              }
            }
            break;
          case INSTRUCTION_TYPE.LOAD_DATA_ATTR:
            {
              ...
              if (typeof value === "undefined") {
                return reject(new Exceptions.UndefinedValueError(attr_name));
              }
          ...
          case INSTRUCTION_TYPE.BUILD_MAP:
            {
              ...
              const iter_count =
                countBy(stack.instructions, (item) => {
                  return (
                    item.level === instr.dataIndex && item.dataIndex !== -1
                  );
                }).true / 2;
              const value = {};
              for (let i = 0; i < iter_count; ++i) {
