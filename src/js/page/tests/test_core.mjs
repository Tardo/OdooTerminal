...
export default class TestCore extends TerminalTestSuite {
  ...
  async test_call_not_named_args() {
    let res = await this.terminal.execute('alias test "print -m \'This is a test! $1 ($2[Nothing])\'"', false, true);
    this.assertIn(res, 'test');
    res = await this.terminal.execute('alias testB -c "print -m \'This is a test! $1 ($2[Nothing])\'"', false, true);
  ...
  async test_alias() {
    ...
    res = await this.terminal.execute('alias -n test -c "print -m \'This is a test! $1 ($2[Nothing])\'"', false, true);
    this.assertIn(res, 'test');
