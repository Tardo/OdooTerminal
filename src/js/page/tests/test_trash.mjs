...
export default class TestTrash extends TerminalTestSuite {
  async test_trash() {
    // Array
    let results = await this.terminal.virtMachine.eval(
      "[1,    2    , 3,     4     ]"
    );
    ...
    results = await this.terminal.virtMachine.eval("[  23,        -2,3,-345]");
    ...
    results = await this.terminal.virtMachine.eval("{keyA: 1234, keyB: 55}");
    ...
    // Global variables
    await this.terminal.virtMachine.eval("$test = 'this is a test'");
    results = await this.terminal.virtMachine.eval("$test");
    this.assertEqual(results[0], "this is a test");
    await this.terminal.virtMachine.eval("$test = 1234");
    results = await this.terminal.virtMachine.eval("$test");
    this.assertEqual(results[0], 1234);
    await this.terminal.virtMachine.eval("$test = [1,2,3,4]");
    results = await this.terminal.virtMachine.eval("$test");
    this.assertEqual(results[0][2], 3);
    await this.terminal.virtMachine.eval("$test[2] = 42");
    results = await this.terminal.virtMachine.eval("$test");
    this.assertEqual(results[0][2], 42);
    await this.terminal.virtMachine.eval(
    ...
    // Runners
    await this.terminal.virtMachine.eval("$test = $(search res.partner)");
    results = await this.terminal.virtMachine.eval("$test['ids']");
    ...
    // Math
    results = await this.terminal.virtMachine.eval("$(((5+5)*2))");
