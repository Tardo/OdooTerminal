...
type ReferenceInfo = [number, string];
...
async function cmdRef(this: Terminal, kwargs: CMDCallbackArgs, ctx: CMDCallbackContext) {
  ...
  for (const xmlid of kwargs.xmlid) {
    if (typeof OdooVerMajor === 'number' && OdooVerMajor < 15) {
      tasks.push(
        callModel<ReferenceInfo>('ir.model.data', 'xmlid_to_res_model_res_id', [xmlid], null, await this.getContext()).then(
  ...
  for (let x = 0; x < len; ++x) {
    ...
    rows.push([item[0], item[1], item[2]]);
  }
