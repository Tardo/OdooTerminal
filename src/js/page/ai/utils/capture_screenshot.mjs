...
export default function captureScreenshot(terminal: Terminal, crop: ?(string | HTMLElement)): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const wasVisible: boolean = termEl.classList.contains('terminal-transition-topdown');
    ...
    function requestCapture() {
      ...
      // Double rAF: first fires before paint, second fires after the browser has
      // actually rendered the hidden terminal, so the screenshot omits it.
    ...
    if (wasVisible) {
      terminal.doHide();
      termEl.addEventListener('transitionend', function onTransEnd() {
        termEl.removeEventListener('transitionend', onTransEnd);
        requestCapture();
      });
    } else {
      requestCapture();
